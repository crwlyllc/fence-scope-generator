  ...
  <div class="sp-container">
    <div class="sp-header">
      ...
      <div class="sp-actions">
        <button class="sp-btn ghost" id="sp-reset">Reset</button>
        <button class="sp-btn" id="sp-add">+ Add Section</button>
        <button class="sp-btn primary" id="sp-complete">Complete → Generate Notes</button>
      </div>
    </div>

    <div class="sp-small" style="margin-bottom:10px">Tip: Use the big + / – buttons on number fields to bump by 1. Add as many sections as you need (front, left side, pool run, etc.).</div>
      ...
      <div class="sp-card">
        <div class="sp-card-header">
          ...
          <div class="sp-row">
            ...
            <button class="sp-btn" id="sp-copy">Copy Notes</button>
          </div>
        </div>
...
(function(){
  ...
  function generateNotes(){
    ...
    state.sections.forEach((s, idx)=>{
      ...
      // Walk gate details per section (new)
    ...
    state.sections.forEach(s=>{
      ...
      const n = parseInt(s.posts2x2||0,10) || 0;
      if(n>0){ posts2x2ByLenCalc[postLen] = (posts2x2ByLenCalc[postLen]||0) + n; }
    });
    ...
    // 3x3 posts as 12ft raw (aggregate)
    ...
    // Concrete bags
    const concreteBagsCalc = totals.posts2x2 + totals.posts3x3 + Math.ceil((totals.postsRealign2x2 + totals.postsRealign3x3) * 0.5) + totals.walkRealign;

    // Gate hardware materials
    const latchCounts = {};
    const hingeCounts = {};
    state.sections.forEach(s=>{
    ...
    // No generic “unpriced” warning anymore unless a true gap remains
  ...
  function persist(){
    try{ syncStateFromDOM(); localStorage.setItem(KEY, JSON.stringify(state.sections)); }catch(e){/* ignore */}
  }
  function restore(){
    try{
      ...
      state.sections = arr.map(s=>{
        const railsCount = s.railsCount || (Array.isArray(s.rails) ? s.rails.length : 2);
        const rails = (s.rails||[]).map(r=>({
          size:r.size,
          stacked:!!r.stacked,
          size2:r.size2,
          patches:parseInt(r.patches||0,10)||0,
          replacements:parseInt(r.replacements||0,10)||0
        }));

        if((parseInt(s.wiRails||0,10)||0) > 0 && rails.every(r=> (parseInt(r.replacements||0,10)||0)===0)){
          if(rails[0]) rails[0].replacements = parseInt(s.wiRails,10) || 0;
        }
        ...
        return {
          ...
          railsCount,
          rails,
          postSize: s.postSize || '2x2',
          postsRealign2x2: parseInt(s.postsRealign2x2 || s.postsRealign || 0, 10) || 0,
          postsRealign3x3: parseInt(s.postsRealign3x3 || 0, 10) || 0,
          walkGates: parseInt(s.walkGates || 0,10) || 0,
          walkGatesRealign: parseInt(s.walkGatesRealign || 0,10) || 0,
          walkGatesHardware: parseInt(s.walkGatesHardware || 0,10) || 0,
          walkGatesRebuild: parseInt(s.walkGatesRebuild || 0,10) || 0,
          walkLatch: s.walkLatch || (GATE.latchTypes && GATE.latchTypes[0]) || 'Gravity Latch',
          walkHinges: s.walkHinges || (GATE.hingeTypes && GATE.hingeTypes[0]) || 'Door Hinges',
          walkRebuildFrame: s.walkRebuildFrame || SEED.railSize[0],
          walkRebuildPicket: s.walkRebuildPicket || SEED.picketSize[0],
          walkRebuildLatch: s.walkRebuildLatch || (GATE.latchTypes && GATE.latchTypes[0]) || 'Gravity Latch',
          walkRebuildHinges: s.walkRebuildHinges || (GATE.hingeTypes && GATE.hingeTypes[0]) || 'Door Hinges',
        };
      });
